"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   WINDCAST  v5.1  —  Lifting Operations Weather Forecast                    ║
...
.wc-nav-btn.active-tab {
  background: rgba(59,130,246,.15);
  color: var(--accent);
  border-color: rgba(59,130,246,.3);
}
.wc-nav-btn.active-mode {
  background: rgba(239,68,68,.12);
  color: var(--stop);
  border-color: rgba(239,68,68,.25);
}
/* Land active style */
.wc-nav-btn.active-land {
  background: rgba(59,130,246,.15);
  color: var(--accent);
  border-color: rgba(59,130,246,.3);
}
...
.legend-strip {
  ...
  padding:.7rem 1rem; background:var(--card); border:1px solid var(--rim); border-radius:10px;
}
.leg-item { display:flex; align-items:center; gap:.4rem;
  font-family:var(--font-h); font-weight:700; font-size:.72rem; letter-spacing:.05em; }
...
.tbl-label small { font-size:.68rem; font-weight:400; color:var(--txt-dim); margin-left:.4rem; }

/* ── Disclaimer ── */
.wc-disclaimer {
  background:rgba(8,15,31,.8); border:1px solid var(--rim); border-radius:8px;
  padding:.55rem .8rem; font-family:var(--font-b); font-size:.65rem;
  color:var(--txt-dim); line-height:1.5; margin-top:.8rem;
}

/* ── Info styles ── */
.info-section { margin-bottom: 1.2rem; }
.info-h { font-family:var(--font-h); font-weight:900; font-size:1rem; letter-spacing:.05em;
  color:var(--accent); border-bottom:1px solid var(--rim); padding-bottom:.4rem; margin-bottom:.7rem; }
...
.info-li { display:flex; gap:.5rem; font-family:var(--font-b); font-size:.82rem;
  color:var(--txt-dim); line-height:1.55; margin-bottom:.3rem; }
...
.info-badge { display:inline-block; background:rgba(59,130,246,.1); border:1px solid rgba(59,130,246,.3);
  border-radius:5px; padding:.15rem .6rem; font-family:var(--font-m); font-size:.72rem;
  color:var(--accent); margin:.1rem .15rem; }

/* ── Ko-fi button ── */
.kofi-btn { display:inline-flex; align-items:center; gap:.5rem; background:#FF5E5B; color:#fff !important;
  font-family:var(--font-h); font-weight:700; font-size:.85rem; letter-spacing:.08em;
  padding:.65rem 1.4rem; border-radius:10px; text-decoration:none !important; margin:.6rem 0; transition:background .15s; }
...
def fetch_offshore_marine(lat, lon, hours=168):
    ...
    try:
        ...
        df = pd.DataFrame({
            ...
            "swell_period": pd.to_numeric(h.get("swell_wave_period",    [np.nan]*n), errors="coerce"),
            "swell_dir":    pd.to_numeric(h.get("swell_wave_direction", [np.nan]*n), errors="coerce"),
        })
        now = pd.Timestamp.now().floor("h")
        return df[df["time"] >= now].reset_index(drop=True)
    except Exception as e:
...
def build_land_rows(df, crane_h, terrain, unit, temp_unit, hours, show_3h=False):
    ...
    for _, row in df.head(hours).iterrows():
        ...
        wd  = row.get("wind_dir", np.nan);       tmp = safe_float(row.get("temperature"))
        prc = safe_float(row.get("precip"));     cld = safe_float(row.get("cloud"))
        ...
        prev_day= day_str
        rc  = rain_row_class(prc); tc, tf = temp_colour(tmp); rlH = risk_level(wg_h)
...
def build_offshore_rows(wind_df, marine_df, crane_h, unit, temp_unit, hours, show_3h=False):
    ...
    for i in range(n):
        ...
        dt_str  = ts.strftime("%a %d %b %H:%M"); day_str = ts.strftime("%Y-%m-%d")
        db_class= " day-break" if day_str != prev_day else ""
        prev_day= day_str
        rc = rain_row_class(prc); tc, tf = temp_colour(tmp); rlH = risk_level(wg_h)
        dir_str = f"{direction_arrow(wd_f)} {wd_f:.0f}°" if not np.isnan(wd_f) else "—"
...
def render_now_card(df, crane_h, terrain, unit, mode, sunrise=None, sunset=None):
    ...
    if mode == "land":
        ...
        ws_h = ws * ((crane_h / 10) ** 0.11); wg_h = wg * ((crane_h / 10) ** 0.11)
    rl  = risk_level(wg_h); tmp = safe_float(row.get("temperature"))
    prs = safe_float(row.get("pressure"), 1013.0); wd = row.get("wind_dir", np.nan)
    try: wd_f = float(wd) if not np.isnan(float(wd)) else np.nan
    except: wd_f = np.nan
    dir_str = f"{direction_arrow(wd_f)} {wd_f:.0f}°" if not np.isnan(wd_f) else "—"
...
<div class="metric-block" style="margin-top:.6rem;">
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.4rem;">
    <div style="display:flex;align-items:center;gap:.5rem;">
      ...
      <div style="font-family:var(--font-m);font-weight:600;font-size:.85rem;color:var(--txt);">{sr_fmt}</div></div>
    </div>
    ...
    <div style="display:flex;align-items:center;gap:.5rem;">
      ...
      <div style="font-family:var(--font-m);font-weight:600;font-size:.85rem;color:var(--txt);">{ss_fmt}</div></div>
      <span style="font-size:.9rem;">🌇</span>
    </div>
  </div>
  <div class="sun-bar"><div class="sun-fill" style="width:{pct}%"></div></div>
            ...
            pass
    gust_h_str  = fmt_wind(wg_h, unit); wind_h_str = fmt_wind(ws_h, unit)
...
<div class="wc-card" style="height:100%;">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:.5rem;margin-bottom:.5rem;">
    <div class="metric-block">
      ...
      <div class="metric-val mv-{rl10}">{gust_10_str}</div>
    </div>
    <div class="metric-block">
...
def render_optimal_window(df, crane_h, terrain, mode):
    if df is None or df.empty: return
    safe_windows = []; current_start = None
    ...
    if safe_windows:
        s, e = safe_windows[0]
        s_str = pd.to_datetime(s).strftime("%H:%M"); e_str = pd.to_datetime(e).strftime("%H:%M")
...
<div class="info-section">
  ...
  <div class="info-li"><span class="info-num">✦</span><span>Offshore / Sea mode — Hs, swell, wave period, IMCA LR006 height correction (α = 0.11)</span></div>
</div>
...
<div class="info-section">
  ...
  <div class="info-li"><span class="info-num">2.</span><span>Set crane height. Choose Land or Sea (Coastal) from the nav bar top-right</span></div>
  <div class="info-li"><span class="info-num">3.</span><span>For Land: choose terrain type — this drives the BS 7121 power-law height correction</span></div>
  <div class="info-li"><span class="info-num">4.</span><span>Click Get Forecast — table loads with hourly speeds at 10m and at your crane height</span></div>
  <div class="info-li"><span class="info-num">5.</span><span>Read the ✦ crane-height column — SAFE ● / CAUTION ⬡ / STOP Ⓧ</span></div>
  <div class="info-li"><span class="info-num">6.</span><span>Always verify with a calibrated on-site anemometer before commencing any lifting operation</span></div>
</div>
...
<div class="info-section">
  ...
  <p class="info-p" style="margin-top:.8rem;font-size:.72rem;">Row background tint = rain intensity — clear / light / moderate / heavy / very heavy.</p>
</div>

<div class="info-section">
  <div class="info-h">🪤 Where's The Catch?</div>
  <p class="info-p">Open-Meteo free tier — full ECMWF IFS resolution for 7 days, updated every 6 hours. For lift planning that is absolutely fine.</p>
  <p class="info-p">This is not a replacement for your anemometer. It never will be.</p>
  <p class="info-p"><strong>Email list:</strong> Want to be notified when tips cover the paid ECMWF API cost? Drop your email in the feedback form below. No mailing list software, no spam — I'll contact everyone individually.</p>
</div>

<div class="info-section">
  <div class="info-h">📦 Version &amp; Changelog</div>
  <span class="info-badge">v5.1 — Current</span>
  <div style="margin-top:.6rem;display:flex;flex-direction:column;gap:.3rem;">
    <div class="info-li"><span class="info-num">▸</span><span><strong>v5.1 — April 2026:</strong> Deduplicated navigation — Forecast / Info / Land / Sea controlled from nav bar only. No repeated buttons.</span></div>
    <div class="info-li"><span class="info-num">▸</span><span><strong>v5.0 — April 2026:</strong> New design system, rain row tints, optimal window, sunrise/sunset bar, 24h/3h toggle, PDF export.</span></div>
    <div class="info-li"><span class="info-num">▸</span><span><strong>v4.0 — April 2026:</strong> Disclaimer, shareable URL, Ko-fi, feedback link, Info section.</span></div>
    <div class="info-li"><span class="info-num">▸</span><span><strong>v3.1 — March 2026:</strong> Land and offshore merged. Combined Go/No-Go across all parameters.</span></div>
    <div class="info-li"><span class="info-num">▸</span><span><strong>v2.1 — Feb 2026:</strong> Swell Hs, visibility, sea temp, air temp added.</span></div>
  </div>
</div>
""", unsafe_allow_html=True)
...
Data: ECMWF IFS 0.25° via Open-Meteo (free tier). v5.1
...
def render_forecast_page(mode):
    ...
    if mode == "land":
        cols = st.columns([3.5, 0.45, 1.0, 2.2, 1.2, 0.8, 1.8])
        ci_loc, ci_pin, ci_h, ci_ter, ci_wu, ci_tu, ci_btn = cols
    else:
        cols = st.columns([3.5, 0.45, 1.0, 1.2, 0.8, 1.8])
        ci_loc, ci_pin, ci_h, ci_wu, ci_tu, ci_btn = cols
    ...
    if saved:
        ...
        picked = st.selectbox("📍 Load saved site", ["— select —"] + saved_names, key="load_saved")
        if picked != "— select —":
    ...
    if fetch_btn or "df_cache" not in st.session_state:
        if mode == "land":
            ...
            st.session_state.sunrise      = None
            st.session_state.sunset       = None
    ...
    # ── Table controls — NO Land/Sea here, they are in the nav bar ───────────
    ...
    # Single row: label | 1D 3D 7D MAX | 24H 3H | PDF | Share
    hcols = st.columns([3.0, 0.55, 0.55, 0.55, 0.55, 0.6, 0.6, 0.7, 0.7])

    with hcols[0]:
        st.markdown(
            f'<div style="font-family:var(--font-h);font-weight:900;font-size:1.1rem;'
            f'color:#fff;padding-top:.4rem;">Forecast '
            f'<span style="font-size:.7rem;font-weight:400;color:var(--txt-dim);">'
            f'{loc_name[:38]} · {updated_str} UTC · {mode_src}</span></div>',
            unsafe_allow_html=True)

    for i, (days, _) in enumerate(DAY_OPTIONS.items()):
        with hcols[i + 1]:
            st.markdown('<div style="margin-top:.3rem;">', unsafe_allow_html=True)
            btype = "primary" if st.session_state.fdays == days else "secondary"
            if st.button(f"{days}D", key=f"day_{days}", type=btype, use_container_width=True):
...
.td-safe { color:#22c55e; } .td-warn { color:#f59e0b; } .td-stop { color:#ef4444; }"""
            pdf_html = (f'<!DOCTYPE html><html><head><meta charset="utf-8"><style>{pdf_css}</style></head><body>'
                        f'<h3 style="color:#3b82f6;margin-bottom:4px;">Windcast — {loc_name}</h3>'
                        f'<p style="color:#7a90b8;font-size:7pt;margin:0 0 6px 0;">'
                        f'Crane {crane_h}m · {forecast_hours}h · {fetch_t.strftime("%Y-%m-%d %H:%M UTC")} · ECMWF IFS 0.25°</p>'
                        f'<table><thead>{hdr_pdf}</thead><tbody>{body_pdf}</tbody></table>'
                        f'<p style="color:#555;font-size:6pt;margin-top:6px;">FOR PLANNING PURPOSES ONLY · BS 7121-1:2016 · LOLER 1998 · HSE PM55 · IMCA LR006</p>'
                        f'</body></html>')
            ...
            st.button("📄 PDF", disabled=True, use_container_width=True,
                      help="pip install weasyprint to enable PDF export")
        st.markdown('</div>', unsafe_allow_html=True)
...
&nbsp;·&nbsp; <a href="{FEEDBACK_URL}" target="_blank" style="color:var(--txt-dim);">📝 Found an error? Tell me here.</a>
    ...
    # ── Advanced (model info) — collapsed at bottom ────────────────────────────
...
Windcast uses ECMWF IFS exclusively — the highest-accuracy freely available global model,
used as the reference standard by meteorological agencies worldwide.
""")
        models_used = st.session_state.get("models_used", ["ECMWF IFS 0.25°"])
        st.caption(f"Active this session: {models_used}")
...
# ══════════════════════════════════════════════════════════════════════════════
# MAIN  — nav bar is the SINGLE source of truth for all page/mode switching
# ══════════════════════════════════════════════════════════════════════════════
def main():
    # ── Session state init ────────────────────────────────────────────────────
    defaults = {
    ...
    # ══ NAV BAR — single source of truth for all navigation ══════════════════
    # Each button updates session_state and reruns — no duplicate controls below.
    fc_class   = "active-tab"  if active_tab == "forecast"  else ""
    inf_class  = "active-tab"  if active_tab == "info"       else ""
    land_class = "active-land" if mode == "land"             else ""
    sea_class  = "active-mode" if mode == "offshore"         else ""

    st.markdown(f"""
<div class="wc-nav">
  <div class="wc-logo">⚡ Wind<span class="accent">cast</span></div>
  <div class="wc-nav-right">
    <button class="wc-nav-btn {fc_class}"
      onclick="window.location.href='?tab=forecast&mode={mode}'"
      id="nav-forecast">🌤️ Forecast</button>
    <button class="wc-nav-btn {inf_class}"
      onclick="window.location.href='?tab=info&mode={mode}'"
      id="nav-info">ℹ️ Info</button>
    <div class="wc-nav-sep"></div>
    <button class="wc-nav-btn {land_class}"
      onclick="window.location.href='?tab={active_tab}&mode=land'"
      id="nav-land">🏗️ Land</button>
    <button class="wc-nav-btn {sea_class}"
      onclick="window.location.href='?tab={active_tab}&mode=offshore'"
      id="nav-sea">⚓ Sea</button>
  </div>
</div>
""", unsafe_allow_html=True)

    # Read URL-driven tab/mode switches (from nav clicks above)
    if "tab" in params:
        new_tab = params["tab"]
        if new_tab in ("forecast", "info") and new_tab != st.session_state.active_tab:
            st.session_state.active_tab = new_tab
            active_tab = new_tab
    if "mode" in params:
        new_mode = params["mode"]
        if new_mode in ("land", "offshore") and new_mode != st.session_state.mode:
            st.session_state.mode = new_mode
            mode = new_mode
            for k in ["df_cache","marine_cache","fetch_time","sunrise","sunset"]:
                st.session_state.pop(k, None)

    # ── DISCLAIMER — shown once before anything else ──────────────────────────
    if not st.session_state.disclaimer_ack:
        ...
        if st.checkbox(
            ...
            "I will verify with a calibrated on-site anemometer before commencing any lifting operation.",
            key="disclaimer_checkbox"):
            st.session_state.disclaimer_ack = True
            st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
        return  # nothing else until acknowledged
