"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   WINDCAST  v5.6  —  Lifting Operations Weather Forecast                    ║
...
@media(max-width:768px){
  ...
  /* Mobile pill toggles — stretch full width */
  .mob-nav-bar div[data-testid="stRadio"]>div[role="radiogroup"]{
    width:100%!important;
    border-radius:8px!important;
  }
  .mob-nav-bar div[data-testid="stRadio"]>div[role="radiogroup"]>label{
    flex:1!important;
...
def place_coords(name):
    try:
        r=requests.get("https://nominatim.openstreetmap.org/search",
                       ...
                       headers={"User-Agent":"Windcast/5.6"},timeout=6)
        d=r.json()
...
<div style="margin-bottom:1.1rem;">
  ...
  <span class="info-badge">v5.6 — Current</span>
...
BS 7121-1:2016 | LOLER 1998 | HSE PM55 | IMCA LR006. Open-Meteo ECMWF IFS 0.25°. v5.6
</div>""", unsafe_allow_html=True)
...
def main():
    ...
    # Two rows: [Forecast | Info] and [Land | Sea] as pill toggles
    # ══════════════════════════════════════════════════════════════════════════
    st.markdown(
        '<div class="mob-nav-bar">'
        '<div class="mob-logo"><span class="bolt">⚡</span> Wind<span class="cast">cast</span></div>',
        unsafe_allow_html=True)

    # Row 1 — Forecast | Info
    tab_opts = ["🌤 Forecast", "ℹ Info"]
    tab_sel  = st.radio("Page", tab_opts,
                        index=0 if active_tab=="forecast" else 1,
                        horizontal=True, key="mob_tab",
    ...
    # Row 2 — Land | Sea
    mode_opts = ["🏗 Land", "⚓ Sea"]
    mode_sel  = st.radio("Mode", mode_opts,
                         index=0 if mode=="land" else 1,
                         horizontal=True, key="mob_mode",
                         label_visibility="collapsed")
    ...
    with c_go:
        # Align button vertically with input using spacer matching label height
    ...
    st.markdown('</div>', unsafe_allow_html=True)  # controls-bar
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # TABLE CONTROLS — all via st.radio (reliable) + st.button for PDF/Share
    # Layout: [Title/meta | dur-radio | res-radio | PDF | Share]
    # ══════════════════════════════════════════════════════════════════════════
    ...
    with tc_dur:
        # ── 1D | 3D | 7D | MAX ── fully working via st.radio
    ...
    with tc_res:
        # ── 24H | 3H ── fully working via st.radio
